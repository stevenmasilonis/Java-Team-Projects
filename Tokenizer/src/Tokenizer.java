...
public final class Tokenizer {
    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        if (!SEPARATORS.contains(String.valueOf(text.charAt(position)))) {
            while (end < text.length()
                    && !SEPARATORS.contains(String.valueOf(text.charAt(end)))) {
                end++;
            }
        } else {
            while (end < text.length()
    ...
    public static Queue<String> tokens(SimpleReader in) {
        ...
        /**
         * Set<Character> strolls = new Set1L<Character>(); for (int i = 0; i <
         * SEPARATORS.length(); i++) { char c = SEPARATORS.charAt(i); if
         * (!strolls.contains(c)) { strolls.add(c); } }
         */
        ...
        while (!in.atEOS()) {
            ...
            while (pos < line.length()) {
                ...
                if (!SEPARATORS.contains(String.valueOf(line.charAt(pos)))) {
